...
Probably they preferred this solution because the other one would be heavy for the server that should verify clients' credentials browsing a large database several times, resulting in a perceptible slowdown in the application layer and a large memory consumption.\\

As we have already seen, a benefit of using client certificates is that authentication is moved from the application layer to the transport one.\\
