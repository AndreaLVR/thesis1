...
\bigskip
%Before describing how we can make SDP working with MQTT, let us clarify that the SDPs we are going to see are not exactly like the one in Figure \ref{fig:sdpstructure}; they are similar, since they follow the idea of ``hide to protect", but simpler.\\
...
Clearly, both the second and third approaches can be seen in two different ways. The customer uses data from its Local Control Room

\begin{itemize}
\setlength{\itemindent}{+4mm}
\item[$\bullet$] and when he (or his ML algorithms) detects an imminent breakdown, he contacts the company assistance
\item[$\bullet$] for other purposes, and let the company predicting possible failures using Centralized Control Room's data.\\
\end{itemize}
...
\item[$\bullet$] \emph{Network Layer}: this layer takes care of all network related operations, such as network setup, end device connection and disconnection to network, routing, device configurations, etc.
%it adds routing capabilities that allows data packets to traverse multiple devices (multiple hops) to route data from source to destination, in order to create more complex topologies.
...
Most of the ZigBee security features we discuss are strictly related to the cryptographic (symmetric) keys used by the protocol; in order to talk about them, we must first take a look at the following list of assumptions~\cite{secanalysisofzigbee} on which they are based:
...
%let us clarify that each node is able to store several network keys, but also to identify the current one with a unique key sequence number assigned by the Trust Center.\\
...
Without entering in too many details, it is important to know that ZigBee and IEEE 802.15.4 try to reduce the presence of interferences using \emph{low RF transmission power}, \emph{low duty cycle} and the \emph{CSMA/CA}  mechanism.\\
...
We will not discuss the following techniques, since they are outside the context, but for completeness I provide a list of some of them:

\begin{itemize}
\setlength{\itemindent}{+4mm}
\item \emph{CSMA/CA} \footnote{https://en.wikipedia.org/wiki/Carrier-sense\_multiple\_access\_with\_collision\_avoidance}
...
\item[$\bullet$] it would be better to always use the commercial mode, which is more secure than the standard one 
\item[$\bullet$] avoid using manufacturer link keys, and remember that they are often public
\item[$\bullet$] prefer unique keys rather than global ones whenever possible
...
\item[$\bullet$] specify the trusted devices with which each device can communicate using its own ACL.
...
The considerations above should help you to understand how to build an almost secure and reliable ZigBee network.\\
In chapter \ref{chap:conclusion} we will quickly discuss the real-world protocol usage, highlighting and summing up its good and bad aspects, which can influence the choice of a potential new user.\\
...
More and more people and companies show interest in MQTT everyday, as confirmed by \emph{Google Trends}, which highlights a strong and continuously growing interest related to the keyword ``MQTT" in the recent years, as we can see in Figure \ref{fig:mqtt_googletrends}.
...
What I think about this protocol is that it is still not considered as important and revolutionary as MQTT, because it is mainly suited for smart home applications and domotic, which are interesting applications but currently still not much in vogue and necessary.\\ 
...
In addition to them, there also many communication malfunctions such as possible interferences with other wireless networks, loss of key synchronization, loss of periodic key updates.\\
...
Finally, technological progress is surprisingly fast and even small devices are becoming more and more powerful and able to support other heavier wireless protocols, like Wi-Fi, which is definitely much more widespread.\\
