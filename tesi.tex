...
\bigskip
\textsc{MQTT} \emph{(Message Queue Telemetry Transport)} is a simple, open and light-weight standarized messaging transport protocol, based on top of TCP/IP, invented by \emph{Andy Standord-Clark} of \emph{IBM} and \emph{Arlen Nipper} of \emph{Cirrus Link Solutions} in 1999, with the following goals:
...
Nodes are arranged around a component, usually called \emph{broker} or \emph{dispatcher}, in a star topology and, as we can see in the figure ~\ref{fig:mqttArchitecture}, they talk to each publishing or subscribing to topics; we are going to discuss them in the section \ref{ssec:topicsandsub}.
...
\end{itemize}\
In the next page, we are going to take a look at a schematic  example of the publish/subscribe pattern combined with topics subscription, but, in order to better understand it, let's keep a reference list of the existent MQTT messages we are going to deal with. Figure~\ref{fig:mqttmsgs} list all of them.
...
\begin{figure}[H]
\includegraphics[width=12cm,height=11cm,keepaspectratio]{pubsub_schema}
\centering
...
This field can assume six different values, where the first one (Connection accepted) indicates the connectin has been accepted, while the others indicate that connection has been refused for some reason (Unacceptable protocol version, Invalid identifier, Server unavailable, Not Authorized, Bad username and password).
...
\begin{itemize}
\setlength{\itemindent}{+4mm}
\item \emph{packetId} is a reference identifier to the current packet
...
As already seen, a client can specify the LWT message as part of the \texttt{CONNECT} packet, then the broker will store the message till it detects that the client has been abruptly disconnected badly, finally topic's subscribed clients will be notified.\\
...
On 10 May 2018 this search produced 52344 results, as shown in figure~\ref{fig:shodanres}:\\
...
While this is already a big number, the more interesting thing is that such a number is growing up very quickly: just to realize, two weeks before the same research gave me about 46 almost thousand devices.\\\\
...
The first clue led us to think that security is neglected is that, if we search again on Shodan, this time filtering on port 8883, we get only 56 servers, so statistically only one broker in a thousand opt for the secure version of the protocol.
...
This behaviour gives an attacker the possibility to connect the broker, requiring a persistence connection, subscribe to as many topics as possible, then disconnect; doing this, the broker will store a huge number of messages to be sent to the client when it reconnects.\\
...
\begin{enumerate}
\setlength{\itemindent}{+4mm}
  \item Since, by default, no encryption system is used, sending credentials is totally pointless, in fact even a trivial \emph{Man In The Middle} attack~\cite{mitmwiki} would be enough to sniff them without effort, as we can see in the image below.
...
\item[$\bullet$] Cons:
\begin{itemize}
\item Managing a huge number of client certificates may be a serious burden, therefore it is important to have a solid certificate provisioning process. In other words, clients' certificates must be kept updated, and owners should have a way to manage their lifecycle: if they can control all their MQTT clients, making possible to update their certificate periodically. It is suggested to use certificates, only when clients are under their strict control.
\item It could happen that a client's certificate get leaked and, consequently, cannot be trusted anymore since some attacker may use it, so it it necessary to have a way to revoke invalid certificates.\\
...
Servers can recognize invalid client certificates consulting their list; clearly, servers have to exchange their lists in order to stay up to date. This is not a very convenient procedure.
...
This is a better and faster solution since you do not have to communicate with other servers to stay up to date; you simply ask your certification authority, which is going to do the boring job for you.\\
...
More in details, the resources to protect are the common actions: publishing, subscribing, setting a LWT message, requiring a persistent session.\\\\
...
This way is secure, because, as already said, the broker ican verify the authenticity of the token by checking its signature.
...
In this section we discuss how those constraint devices may encrypt only specific data, like the payload in the \texttt{PUBLISH} message, or credentials within the \texttt{CONNECT} packet.\\
...
\begin{itemize}
\setlength{\itemindent}{+4mm}
\item[$\bullet$] \emph{Checksum/Hash}: a value calculated, with a series of calculations that converts the message payload into a string of digits.\\
...
\item[$\bullet$] \emph{Message Authentication Code (MAC)}: this authentication code (usually \emph{HMAC}) is calculated using a cryptographic Hash function combined with a cryptographic key. Since only trusted clients know the secret key, we can discover if a message came from a trusted client or not.\\
HMAC is very fast compared to digital signatures, but the con is that all the clients, both publishers and subscribers, share the same key and, besides, there must be a way to send it to them.
...
The difference with respect to the previous approach is that the secret key was shared to all clients, while in this case each client has its own private key, but the broker is still able to decrypt their messages using the public key it received.\\
\end{itemize}
